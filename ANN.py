...
class NeuralNetwork:
    def __init__(self, X, Y, X_test, Y_test, hidden_nodes=20, learninig_rate=0.2, epochs=6000):
        # data
    ...
    def train(self, epochs):
        ...
        ann_output['Error'] = (abs(error)).mean()
    ...
    def test(self):
        ...
        # print('Test Accuracy : ', ((correct / len(self.Y_test)) * 100), '%')
        ann_output['Test Accuracy'] = ((correct / len(self.Y_test)) * 100)

        precision, recall, fscore, support = precision_recall_fscore_support(self.Y_test, pred_list, average=None)

        tn, fp, fn, tp = confusion_matrix(self.Y_test, pred_list).ravel()
        # print('True Nagative', tn)
        ann_output['True Nagative'] = tn

        # print('False Positive', fp)
        ann_output['False Positive'] = fp

        # print('False Negative', fn)
        ann_output['False Negative'] = fn

        # print('True Positive', tp)
        ann_output['True Positive'] = tp

        total = tn + tp + fn + fp

        # print('Test Accuracy : ', (tn + tp)/total)
        ann_output['Test Accuracy'] = (tn + tp) / total

        # print('Missclassification Rate : ', (fn + fp)/total)
        ann_output['Missclassification Rate'] = (fn + fp) / total

        # print('precision : ', precision)
        ann_output['precision'] = precision

        # print('recall : ', recall)
        ann_output['recall'] = recall

        # print('FScore : ', fscore)
        ann_output['FScore'] = fscore

        print('Support : ', support)
        ann_output['Support'] = support

        # print('Sensitivity or TPR : ', (tp/ (tp + fn)) )
        ann_output['Sensitivity or TPR'] = (tp / (tp + fn))

        # print('Specificity or TNR : ', (tn/(tn+fp)))
        ann_output['Specificity or TNR'] = tn / (tn + fp)

        # print('False Positive Rate or Fallout : ', (fp/(fp+tn)))
        ann_output['False Positive Rate or Fallout'] = (fp / (fp + tn))

        # print('False Negative Rate : ', (fn/(fn+tp)))
        ann_output['False Negative Rate'] = (fn / (fn + tp))

        # print('False Discovery Rate : ', (fp/(tp+fp)))
        ann_output['False Discovery Rate'] = (fp / (tp + fp))

        cm = confusion_matrix(self.Y_test, pred_list)
        # return cm
