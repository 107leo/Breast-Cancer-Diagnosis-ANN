...
def second():
   ...
   ############################################### plt.show()
    ...
    def StochasticGradient(train, label, test, label_test):
        ...
        #     float(confusion_mat[0, 0] + confusion_mat[0, 1]) * 100 / confusion_mat.sum()))
        stochastic_output['Accuracy of Stochastic Gradient classifier on test set'] = float(
            confusion_mat[0, 0] + confusion_mat[1, 1]) * 100 / confusion_mat.sum()

        # print(confusion_mat)
        stochastic_output['Confusion Matrix'] = confusion_mat

        # print('Time took for training and predicting the results {0:.5} in seconds\n'.format(float(end - start)))
        stochastic_output['Time took for training and predicting the results in seconds'] = float(end - start)
        ...
        # print('Recall ', recall)
        stochastic_output['Recall'] = recall

        # print('F1- Score  ', f_score)
        stochastic_output['F1- Score'] = f_score
        ...
        stochastic_output['Area Under the Curve'] = roc_auc4
    ...
    def SupportVectorMachines(train, label, test, label_test):
        ...
        #     float(confusion_mat1[0, 0] + confusion_mat1[0, 1]) * 100 / confusion_mat1.sum()))
        svm_output['Accuracy of Support vector Machines classifier on test set'] = float(
            confusion_mat1[0, 0] + confusion_mat1[1, 1]) * 100 / confusion_mat1.sum()

        # print(confusion_mat1)
        svm_output['Confusion Matrix'] = confusion_mat1

        # print('Time took for training and predicting the results {0:.5} in seconds\n'.format(float(end - start)))
        svm_output['Time took for training and predicting the results in seconds'] = float(end - start)
        ...
        svm_output['Recall'] = recall1

        # print('F1- Score  ', f_score1)
        svm_output['F1- Score'] = f_score1
        ...
        # print('Area Under the Curve ', roc_auc4)
        svm_output['Area Under the Curve'] = roc_auc4
    ...
    return svm_output, stochastic_output
